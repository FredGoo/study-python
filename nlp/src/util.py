...
def fullcut(content):
    ...
    word_list = []
    if not GlobalParament.ruler_list:
        r = r'[^/]{2,}'
...
def halfcut(content):
    ...
    while True:
        ...
        if not GlobalParament.ruler_list:
            ...
            for word in word_list_temp:
                if word not in GlobalParament.ruler_list:
                    word_list.append(word)
                    # print len(word_list)
        if (len(word_list) >= GlobalParament.n):
            break
        else:
            word_list = []
            k += 1
...
def prepro_file(fl_in_url, re_out_url, *wd_be_del):
    ...
    for file in fl_in:
        ...
        if os.path.isfile(afile_url):
            ...
            con_unicode = StrToUni(content, *(GlobalParament.InputFormatList))
            if GlobalParament.pattern == "full":
                cut_result = fullcut(con_unicode)
            else:
                cut_result = halfcut(con_unicode)
            s_fl_Name = UniToStr(file, *(GlobalParament.OutputFormatList))
            re_out.write(s_fl_Name + '\t')
            key_word_out = []
            for key_word in cut_result:
                s_key_word = UniToStr(key_word, *(GlobalParament.OutputFormatList))
...
def TF_IDF_Compute(file_import_url_temp):
    ...
    newpath2 = GlobalParament.newpath2
